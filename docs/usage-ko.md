...
그다음 로컬 브라우저에서 `http://127.0.0.1:8765/`을 엽니다. 외부 자산과 분석 스크립트는 없습니다. 기록기는 논리 식별자, 상태, 의존성, 알려진 역할·모델·추론 수준, 태스크 제목, 최대 160자의 설명처럼 허용 목록에 있는 정제된 생명주기 정보만 저장합니다. `prompt`, 응답 내용, `outputFile`, 사용량 원격 측정(usage telemetry), transcript 경로, 코드, 명령, 도구 출력, 환경 변수, 인증정보와 모델 목록은 저장하지 않습니다. 정제된 상태만 `${XDG_STATE_HOME:-~/.local/state}/claudex5-engineering-harness/runs` 아래에 디렉터리 `0700`, 파일 `0600` 권한으로 해당 머신에 저장됩니다.
