...
- Use `harness_spark_ui_iteration` only when it is registered and the request is exactly one small, bounded change to an existing user interface with explicit acceptance criteria and UI verification. If it is not registered or the scope expands, keep the current primary implementation route. Never use Spark for architecture, backend logic, security-sensitive behavior, data migration, broad refactors, or review.
